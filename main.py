...
def main():
    ...
    def show_users():
        ...
        query = "SELECT name,surname,posts,location,st_astext(coords),id FROM public.users ORDER BY id ASC"
    ...
    def add_user() -> None:
        ...
        cursor = db_params.cursor()
        query = f"INSERT INTO public.users (name, surname, posts, location, coords) VALUES ('{name}', '{surname}', {posts}, '{location}', 'SRID=4326;POINT({latitude} {longitude})');"
        cursor.execute(query)
        db_params.commit()
        cursor.close()
